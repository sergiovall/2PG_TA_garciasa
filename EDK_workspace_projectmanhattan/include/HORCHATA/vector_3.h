...
inline float Vector3::Angle(const Vector3& a, const Vector3& other)  {
	float aux = sqrtf(a.x * a.x + a.y * a.y + a.z * a.z) * sqrtf(other.x * other.x + other.y * other.y + other.z * other.z);
	return acosf((a.x*other.x + a.y * other.y + a.z * other.z)/aux);
}

inline Vector3 Vector3::CrossProduct(const Vector3& a, const Vector3& other)  {
	return Vector3(a.y * other.z - a.z * other.y,-(a.x * other.z - a.z * other.x),a.x*other.y - a.y * other.x);
}
...
inline Vector3 Vector3::Lerp(const Vector3& a, const Vector3& b, float t) {
	//assert(t >= 0 && t <= 1, "lerp value out of range");
	if (t < 0.0f) {
		t = 0.0f;
	}
	else if(t>1.0f){
...
inline Vector3 Vector3::Reflect(const Vector3& direction, const Vector3& normal) {
	return direction.operator-(normal.operator*(DotProduct(direction, normal)));
}
